...
public class Grapple : MonoBehaviour
{
    ...
    public Gradient succeedGradient;
    public Gradient failedGradient;
    ...
    void GrappleShoot()
    {
        ...
        else if(hitInfo && hitInfo.collider.CompareTag("Spikes"))
        {
            //hit something
            grapplePoint = hitInfo.point;
    ...
    public void DoLine(bool isGrappling = false)
    {
        if (!isDoingLine)
        {
            ...
            lineRenderer.colorGradient = succeedGradient;
            StartCoroutine(LineRoutine(isGrappling));
        }
    }
    public void DoFailedLine(bool isGrappling = false)
    {
        if (!isDoingLine)
        {
            Debug.Log("failed grapple");
            lineRenderer.enabled = true;
            lineRenderer.colorGradient = failedGradient;
            StartCoroutine(FailedLineRoutine(isGrappling));
        }
    }
    ...
    IEnumerator FailedLineRoutine(bool isGrappling)
    {
        isDoingLine = true;
